...
namespace GeometryFigures3
{
    public partial class form_addingFigure : Form
    {
        ...
        private void btn_OK_Click(object sender, EventArgs e)
        {
            try
            {
                if (panel_circle.Visible)
                {
                    var coords = tb_circXY.Text.Split(',');
                    var x = int.Parse(coords[0]);
                    var y = int.Parse(coords[1]);
                    var radius = int.Parse(tb_circRadius.Text);

                    Figures.Add(new Circle(new Point(x, y), radius));
                }
                else if (panel_rectangle.Visible)
                {
                ...
                else if (panel_triangle.Visible)
                {
